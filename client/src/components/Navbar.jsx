 ...
 const {user, setShowLogin} = useContext(AppContext)
  ...
  return (
    <div className='flex items-center justify-between py-4'>
      <Link to="/">
        <img src={assets.logo_finall} alt="" className="w-28 sm:w-32 lg:w-40" />
      </Link>
      <div>
        {
        user ?
       <div className="flex items-center gap-2 sm:gap-3">
        <button onClick={()=>navigate('/buy')} className="flex items-center gap-2 bg-blue-100 px-4 sm:px-6 py-1.5 sm:py-2.5 rounded-full hover:scale-105 transition-all duration-700">
